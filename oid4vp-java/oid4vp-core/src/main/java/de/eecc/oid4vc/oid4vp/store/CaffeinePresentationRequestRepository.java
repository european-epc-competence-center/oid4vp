...
/**
 * Caffeine-backed in-memory {@link PresentationRequestRepository}.
 */
public class CaffeinePresentationRequestRepository implements PresentationRequestRepository {

    private static final Logger log = LoggerFactory.getLogger(CaffeinePresentationRequestRepository.class);
    ...
    public CaffeinePresentationRequestRepository(Duration ttl) {
    ...
    @Override
    public <T extends PresentationRequest> void saveAfterComplete(T request) {
        byState.put(request.getState(), request);
        log.debug("Saved OID4VP request after complete direct_post state={}", request.getState());
    }
