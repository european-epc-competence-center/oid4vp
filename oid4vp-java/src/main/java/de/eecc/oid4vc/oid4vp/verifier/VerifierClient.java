...
public class VerifierClient {
    ...
    public VerifierClient(String verifierUrl) {
        this(verifierUrl, HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(10)).build(), new ObjectMapper());
    }

    /**
     * Verifies a presentation against the configured vc-verifier.
     *
     * @param presentationNode presentation from {@code vp_token} (string or object)
     * @param nonce            expected challenge
     * @param clientId         expected audience (domain)
     * @return verification outcome including the raw verifier JSON payload
     */
    public PresentationVerificationResult verifyPresentation(JsonNode presentationNode, String nonce, String clientId) {
    ...
    /**
     * Builds the vc-verifier POST body: a one-element array of verifiables.
     */
    public List<Object> buildVerifierRequestBody(JsonNode presentationNode) {
        ...
        throw new Oid4VpException(400, "Unsupported vp_token presentation node type: " + presentationNode.getNodeType());
    }
